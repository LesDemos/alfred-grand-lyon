...
TwitBot.prototype.getUserProfile = function(id, name, callback) {
    T.get('users/show', {user_id: id, screen_name: name}, function(err, data, response) {
      callback(err, data);
    });
}
...
TwitBot.prototype.tweetEvent = function(tweet) {
    ...
    report.username = tweet.user.screen_name;
