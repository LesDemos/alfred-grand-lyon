...
legend.onAdd = function (map) {
  ...
  div.innerHTML =
  '<img src="static/resources/State_Untreated_icon.png" alt="Untreated" style="width:20px;height:20px;">' + '<span>' + table_state[UNTREATED] + '</span>' + '<br>' +
  '<img src="static/resources/State_In_Progress_icon.png" alt="In progress" style="width:20px;height:20px;">' + '<span>' + table_state[IN_PROGRESS] + '</span>';
  return div;
};
...
function getInfoTwitter(feature, layer) {
  fetch(window.Viz.apiEndpoint + '/user?user_id=' + feature.properties.user_id + '&username=' + feature.properties.username, {
    method: 'GET',
    headers: {'Content-Type': 'application/json; charset=UTF-8'}
  }).then(
    function(response) {
