...
const CMD = {
  MENU: {
    ...
    POSTBACK: 'GREETING_MENU'
  },
  REPORT: {
    TEXT: ['SIGNALER', 'SIGNALE', 'INCIDENT'],
    POSTBACK: 'GREETING_REPORT'
  }
};
...
FBGreeting.prototype.unknownCommand = function(options) {
  ...
  // TODO : Add image to quick replies

  this.bot.botly.sendText({
    id: options.user_id,
    text: text,
    quick_replies:[
      this.bot.botly.createQuickReply("Menu", CMD.MENU.POSTBACK),
      this.bot.botly.createQuickReply("Signaler", CMD.REPORT.POSTBACK)      
    ]
  });
