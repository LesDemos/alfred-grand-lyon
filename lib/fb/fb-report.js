...
FBReport.prototype.handleUserMessage = function(options) {
  if(options.data.attachment) {
    options = options || {};
    const images = null;
    console.log(options.data.attachment);
    console.log(options.data.attachment.image);
    console.log(options.data.attachment.image[0]);
    images = options.data.attachment.image;
    if (images != null) {
